...
class WC_PT_Data {
	...
	/**
	 * Get debug data for a product to understand why tabs are not showing.
	 *
	 * @param int $product_id Product ID.
	 * @return array<string, mixed>
	 */
	public function get_debug_data( $product_id ) {
		$debug = [
			'product_id' => $product_id,
			'has_managed_cat' => $this->product_has_managed_category( $product_id ),
			'regular_pos_id' => $this->get_regular_product_pos_id( $product_id ),
		];

		if ( ! function_exists( 'get_field' ) ) {
			$debug['error'] = 'get_field does not exist';
			return $debug;
		}

		$stock_status = sanitize_key( (string) get_post_meta( (int) $product_id, '_stock_status', true ) );
		$debug['stock_status_meta'] = $stock_status;
		$debug['product_available'] = in_array( $stock_status, [ 'instock', 'onbackorder' ], true );

		$raw_categories = get_field( 'categories', $product_id );
		$debug['raw_categories'] = $raw_categories;
		
		if ( empty( $raw_categories ) ) {
			$debug['error'] = 'empty raw_categories from ACF';
			return $debug;
		}
		...
		$categories = array_map(
			...
			(array) $raw_categories
		);
		$debug['mapped_categories'] = $categories;

		$tabs = [];
		$debug['flakony_cat_id'] = $this->settings->get_category_id( 'flakony' );
		if ( in_array( $this->settings->get_category_id( 'flakony' ), $categories, true ) ) {
			$variants = $this->get_variants_from_acf( 'flakony', $product_id, $debug['product_available'] );
			$debug['flakony_variants'] = $variants;
			if ( ! empty( $variants ) && $this->has_available_variants( $variants ) ) {
				$tabs['flakony'] = true;
			} else {
				$debug['flakony_no_available_variants'] = true;
			}
		}

		$debug['zalyszky_cat_id'] = $this->settings->get_category_id( 'zalyszky' );
		if ( in_array( $this->settings->get_category_id( 'zalyszky' ), $categories, true ) ) {
			$variants = $this->get_variants_from_acf( 'zalyszky', $product_id, $debug['product_available'] );
			$debug['zalyszky_variants'] = $variants;
			if ( ! empty( $variants ) && $this->has_available_variants( $variants ) ) {
				$tabs['zalyszky'] = true;
			} else {
				$debug['zalyszky_no_available_variants'] = true;
			}
		}

		$debug['rozpyv_cat_id'] = $this->settings->get_category_id( 'rozpyv' );
		if ( in_array( $this->settings->get_category_id( 'rozpyv' ), $categories, true ) ) {
			$rozpyv_status = $this->normalize_variant_status( get_field( 'rozpyv_status', $product_id ) );
			$rozpyv_price_per_ml = $this->to_float( get_field( 'rozpyv_price', $product_id ) );
			$base = [
				'status'    => $rozpyv_status,
				'price_per_ml' => $rozpyv_price_per_ml,
				'available' => $debug['product_available'] && 'in_stock' === $rozpyv_status && $rozpyv_price_per_ml > 0,
			];
			$debug['rozpyv_base'] = $base;
			...
			if ( ! $this->has_available_rozpyv_options( $rozpyv_tab ) ) {
				$debug['rozpyv_no_available_options'] = true;
			} else {
				$tabs['rozpyv'] = true;
			}
		}

		$debug['final_tabs_found'] = array_keys($tabs);
		return $debug;
	}
}
